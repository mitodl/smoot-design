...
const AiChat: React.FC<AiChatProps> = function AiChat({
  ...
  const initialMessagesWithIds = React.useMemo(() => {
    const prefix = Math.random().toString().slice(1)
    return initialMessages.map((m, i) => ({ ...m, id: `${prefix}-${i}` }))
  }, [initialMessages])
  const {
    messages,
    input,
    ...
    initialMessages: initialMessagesWithIds,
  })
